(function (global, factory) {
    ...
    function compileToFunctions(template) {
      ...
      console.log(code); // 4.将字符串变成函数 限制取值范围 通过with来进行取值 稍后调用render函数就可以通过改变this 让这个函数内部取到结果了
    ...
    function createElm(vnode) {
      var tag = vnode.tag,
          ...
          key = vnode.key,
          data = vnode.data,
          text = vnode.text; // 创建元素 放到vnode.el上
    ...
    function initMixin(Vue) {
      ...
      Vue.prototype.$mount = function (el) {
        ...
        if (!options.render) {
          ...
          var render = compileToFunctions(template); //render = new Function(`width(this){_c(,_v(),...)}`)
